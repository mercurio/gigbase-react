 ...
 *   }
 *
 * v2 of the loader, corresponding to the v2 schema
}
 */
const adminEmail = 'neon@leadbone.com';
// prisma  const endpoint = 'http://localhost:4466';
 ...
 */
async function createPerformance(gig, song, date, dtag, ktag) {
    let dt = fixDate(date);

    response = await query({
        query: `
            mutation CreatePerformance($gig: ID!, $song: ID!, $date: DateTime!) {
                createPerformance(
                    songId: $song
                    gigId: $gig
                    date: $date
...
async function processSong(record) {
    ...
    try {
      var dtag = await createTag(drumkit, record.drumkit);
    } catch(err) {
      console.log("dtag error: " + err.toString())
      return "fail: " + JSON.stringify(record) + '\n';
    }
    try {
      var ktag = await createTag(key, record.key);
    } catch(err) {
      console.log("ktag error: " + err.toString())
      return "fail: " + JSON.stringify(record) + '\n';
    }
    try {
      var perf = await createPerformance(gig, song, record.played, dtag, ktag);
    } catch(err) {
      console.log("perf error: " + err.toString())
      return "fail: " + JSON.stringify(record) + '\n';
    }
